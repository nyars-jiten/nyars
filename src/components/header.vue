<template>
	<header class="sticky top-0 z-50 shadow-md bg-white">
		<div class="max-w-4xl container mx-auto flex flex-wrap py-5 flex-row items-center">
			<div class="mr-4 py-1 pr-4 border-r border-gray-100">
				<a class="flex title-font font-medium items-center bg-logo w-10 h-10 rounded-full bg-cover" />
			</div>

			<nav class="flex flex-wrap items-center text-base justify-center gap-5">
				<a href="#" class="hover:text-red-900">Главная</a>
				<a href="#" class="hover:text-red-900">Багтреккер</a>
				<a href="#" class="hover:text-red-900">Скачать</a>
			</nav>
		</div>
	</header>
</template>

<script setup lang="ts"></script>

<style scoped></style>
