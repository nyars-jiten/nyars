  ...
  <div class="search-item">
    <v-container v-if="entry" :class="{ nonreviewed: !entry.isReviewed }">
      ...
      <div class="words-header">
        <router-link class="titles-link" :to="{ name: 'view-jp', params: { wid: entry.wid } }" >
            <div class="word" v-for="(word, wordId) in entry.entry.words" :key='wordId'>
                <div class="word-writings" v-if="word.writings.length > 0">
                    <div class="writing" v-for="writing in word.writings" :key='writing.id'>
                    {{ writing.value }}
                    <InlineTag v-if="writing.tag" v-bind:tags="writing.tag.values" :short="true" :lang="'rus'" :inf="true" />
                    </div>
                </div>
                <div class="word-readings">
                    <div class="reading" v-for="reading in word.readings" :key='reading.id'>{{ convertR(reading.value) }}
                    <InlineTag v-if="reading.tag" v-bind:tags="reading.tag.values" :short="true" :lang="'rus'" :inf="true" />
                    </div>
                </div>

                <div class="corpus-icon" v-if="!entry.isReviewed && entry.theme && entry.theme.length > 0 && wordId == 0">
                    <DictionaryIcon color="blue lighten-3" :icon="corpusInfo(entry).icon" :tooltip="corpusInfo(entry).description" />
                </div>
            </div>
        </router-link>
      </div>
        <div class="meaning-block">
            <div class="pos-meaning" v-for="pos in entry.entry.meanings" :key="pos.id" >
                <!-- <div class="pos-list">
