...
const optionsList = ['shou', 'chuu', 'kou', 'kanon', 'touon', 'goon', 'kanyoon', 'souon', 'gai']
...
<template>
  <div v-if="readings.length > 0 || isEditor" class="grid grid-cols-[auto_1fr] gap-2">
    <span
    ...
    <div v-if="!isEditor" class="flex flex-row flex-wrap gap-x-1 gap-y-2">
      <div
      ...
      >
        <span v-if="reading.tags.length > 0 && !reading.tags.includes('gai')" class="mx-1">
    ...
    <div v-else class="gap-x-1 gap-y-2">
      <div v-for="(reading, ri) in readings" :key="ri" class="flex gap-2">
        <EditInput v-model="reading.value" name="reading-input" placeholder="чтение" />
        <Multiselect
        ...
        />
        <span class="ml-1 cursor-pointer font-medium text-red-700" @click="readings.splice(ri, 1)"
          >X</span
        >
      </div>
      <Button class="w-full font-medium" @click="addReading">+ чтение</Button>
    </div>
  </div>
</template>
