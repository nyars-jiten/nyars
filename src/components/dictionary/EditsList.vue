    ...
    <v-expansion-panel class="" v-for="edit in lastEdits" :key="edit.id">
      ...
      >
        <div class="titles">
          ...
          <router-link class="titles-link" :to="{ name: 'view-jp', params: { wid: edit.identifier } }" v-if="edit.identifier">
          ...
          <EditTitle v-bind:edit="edit" v-else />
            ...
            <div class="approver" v-if="edit.status != 4 && edit.status != 1">
              <v-avatar size="22px" class="avatar"
                ><img
                  :src="avatarLink(edit.approver.avatar)"
              /></v-avatar>
          ...
          <div class="subheader">
            <v-row no-gutters align="center">
              ...
              <span class="dict-type">{{ getDictType(edit.dictionary) }},</span>
              <span
                class="edit-type"
              ...
              >
              <v-spacer></v-spacer>
              <span class="time">создана {{ timeDif(edit.createdDate) }}</span>
              <v-avatar size="22px" class="avatar"
                ><img
                  :src="avatarLink(edit.author.avatar)"
              /></v-avatar>
      ...
      <v-expansion-panel-content>
        <v-row no-gutters justify="space-between">
              ...
              <div class="side">
                ...
                >
                  <template v-if="result.isDiffered">
                    <div class="new" :key="resultIndex" v-text="result.value" />
                  </template>
          ...
          <v-col md="1" v-if="edit.status == 1">
            <div class="btns">
              <v-btn class="ma-2" outlined fab x-small color="green" v-if="userRoleId >= 2" v-on:click="acceptEdit(edit.id)">
                <v-icon>mdi-check</v-icon>
              </v-btn>
              <v-btn class="ma-2" outlined fab x-small color="red" v-if="userRoleId >= 2" v-on:click="declineEdit(edit.id)">
                <v-icon>mdi-close</v-icon>
              </v-btn>
              <v-btn class="ma-2" outlined fab x-small color="primary" v-if="canUserEdit(edit)" v-on:click="openEditEditor(edit.id)">
...
export default {
  ...
  methods: {
    ...
    canUserEdit(edit) {
      return this.userRoleId >= 2 || (this.userRoleId == 1 && edit.author.id == this.currentUser.id);
    },
