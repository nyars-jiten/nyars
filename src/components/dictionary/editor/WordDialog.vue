            ...
            <v-card class="">
              ...
              <v-card-text>
                <v-container v-if="currentObj" >
                      <!-- {{currentObj}} -->
                  <v-row v-for="(writing, writingIndex) in currentObj.writings" :key="writing.id">

                        <v-col cols="12" md="6" >
                            <v-row >
                                <v-col cols="12" md="1" >
                                    <v-btn icon x-small @click="removeFromWriting(writingIndex)" color="red"><v-icon>mdi-close</v-icon></v-btn>
                                </v-col><v-col cols="12" md="11" >
                                    <v-text-field v-model="writing.value" label="Написание" class="writing-input"></v-text-field>
                                </v-col>
                            </v-row >
                        </v-col>
                        <v-col cols="12" md="6">
                            ...
                            :type="'Kinf'"
                            @changeTags="(...args)=>changeWritingTags(writingIndex,...args)" />
                        </v-col>
                  </v-row>
                  <div class="dial-btn">
                    <v-btn outlined small @click="addWriting()" color="primary"><v-icon>mdi-plus</v-icon> написание</v-btn>
                  </div>
                  <v-divider style="padding: 20px"></v-divider>
                  <v-row v-for="(readings, rIndex) in currentObj.readings" :key="readings.id">
                        <v-col cols="12" md="6" >
                            <v-row >
                                <v-col cols="12" md="1" >
                                    <v-btn icon x-small @click="removeFromReading(rIndex)" color="red"><v-icon>mdi-close</v-icon></v-btn>
                                </v-col><v-col cols="12" md="11" >
                            <v-text-field v-model="readings.value" label="Чтение" class="writing-input"></v-text-field>
                            </v-col>
                            </v-row >
                        </v-col>
                        <v-col cols="12" md="6">
                  ...
                  <div class="dial-btn">
                    <v-btn outlined small @click="addReading()" color="primary"><v-icon>mdi-plus</v-icon> чтение</v-btn>
                  </div>
                </v-container>
              </v-card-text>
...
export default {
    ...
    methods: {
        ...
        changeWritingTags(index, values) {
            const newValue = {value: this.currentObj.writings[index].value, tag: {type: 'Kinf', values}};
            Vue.set(this.currentObj.writings, index, newValue);
        },
        changeReadingTags(index, values) {
            const newValue = {value: this.currentObj.readings[index].value, tag: {type: 'Rinf', values}};
        ...
        addReading() {
            this.currentObj.readings.push({value: "", tag: {type: 'Rinf', values: []}});
        },
        addWriting() {
            this.currentObj.writings.push({value: "", tag: {type: 'Kinf', values: []}});
        },
        updateModel () {
    ...
    // },
    computed: {
        ...mapGetters(['currentEntry', 'dialogModel', 'dialogIndex', 'getSubjectTags']),
        currentObj: function() {
