            ...
            <v-row>
              <v-col cols="12" md="12">
                <v-textarea
                  ...
                  counter
                ></v-textarea>
                <BBCodePanel textAreaId="sense-area" @changed="(...args) => sense.value = args[0]" />
              </v-col>
              <v-col cols="12" md="12">
                <TagSelector
              ...
              <v-col cols="12" md="6">
                <TagSelector
                  ...
                  :type="'Dial'"
                  @changeTags="
                    (...args) => updateTags('Dial', ...args)
                  "
                />
              </v-col>
              <v-col cols="12" md="12" v-for="(example, exampleId) in sense.examples" :key="example.id">
                <v-row>
                <v-col cols="12" md="1" >
                  <v-btn icon x-small @click="removeExample(exampleId)" color="red"><v-icon>mdi-close</v-icon></v-btn>
...
export default {
  ...
  methods: {
    ...
    validateTag(type) {
      if (typeof this.sense.tags == 'undefined' || this.sense.tags.length < 1) return [];
