...
export default {
    methods: {
        processbb(openTag, closedTag) {
            var txtarea = document.getElementById(this.textAreaId);
            txtarea.focus();
            var start = txtarea.selectionStart;
            var finish = txtarea.selectionEnd;
            var allText = txtarea.value;
            var sel = allText.substring(start, finish);
            var newText=allText.substring(0, start)+ openTag +sel+closedTag+allText.substring(finish, allText.length);
