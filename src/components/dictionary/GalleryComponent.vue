  ...
  <div class="gallery">
    <v-card class="" elevation="2" outlined tile v-if="editMode || currentImages.length > 0">
      ...
      <div class="gallery-body">
        <v-container fluid>
          <v-row>
            <v-col cols="12" md="2" sm="4" xs="6" v-for="(img, imgId) in currentImages" :key="imgId">
              <v-img :src="img.link" aspect-ratio="1" class="gallery-img">
        ...
        <v-btn text outlined small @click="addNewImage" color="primary" v-if="editMode"> <v-icon>mdi-plus</v-icon> изображение </v-btn>
      </div>
    </v-card>
  </div>
</template>
