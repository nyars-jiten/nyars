        ...
        <v-col cols="12" md="12">
          <v-card class=" mx-auto" tile>
            ...
            <v-card-title>Результат поиска</v-card-title>
              ...
              <v-card-text class="no-results" v-if="!currentSearchResult.result || currentSearchResult.result.length == 0">
                <p>Ничего не найдено</p>
              </v-card-text>
              <v-list-item class="res-item" v-for="entry in currentSearchResult.result" :key="entry.id">
              ...
              <v-card-actions>
                <v-btn v-if="currentSearchResult.result && currentSearchResult.result.length == 20"
                  color="primary"
                  dark
                  outlined
                  @click="search()"
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      window.scrollTo(0, 0);
      this.startSearch({ request: this.request, page: this.page + this.searchPageOffset });
    }
  },
  computed: {
