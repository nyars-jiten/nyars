          ...
          <v-card elevation="2" outlined tile>
            <v-card-title>
              <div class="title-card">Японско-русская статья</div>
                ...
                <div class="icon-wrapper" v-if="!editMode && !currentEntry.isReviewed">
                  <DictionaryIcon
                    color="warning"
                ...
                <div class="icon-wrapper" v-if="!editMode && userRoleId >= 2" @click="changeStatus">
                  <DictionaryIcon
                    color="grey"
              ...
              <template>
                <v-row>
                  ...
                  <div class="edit-content" v-else>
                      <JapEntryEdit v-bind:entry='currentEntry' v-if="!textMode" />
              ...
              <div class="edit-actions" v-if="editMode">
                <div class="visual-mode" v-if="!textMode">
                  <div class="edit-action pr-2 pl-4">
                    <v-btn color="primary" dark outlined @click.stop="saveEdit">Сохранить</v-btn>
                  </div>
                  <div class="edit-action pl-2">
                    <v-btn color="primary" dark outlined @click.stop="changeToTextMode">Текстовый редактор</v-btn>
                  </div>
                  <div class="edit-action pl-2">
                ...
                <div class="text-mode-actions" v-else>
                  <div class="edit-action pl-2">
                    <v-btn color="primary" dark outlined @click.stop="changeToVisualMode">Вернуться в визуальный редактор</v-btn>
                  </div>
                </div>
              </div>
            </v-card-actions>
...
export default {
    data() {
      return {
        editMode: false,
        textMode: false,
        emptyEntry: { entry: { words: [{"writings":[],"readings":[]}], meanings: [{pos: ["unc"], langMeanings: []}] } },
    ...
    methods: {
      ...mapActions(["getCurrentEntry", "fetchTags", "getCurrentSounds", "newAlert", "deserializeEntry"]),
      changeToVisualMode() {
        this.deserializeEntry()
        this.textMode = false
      },
      async changeStatus() {
        const resp = await sendPostRequest('dictionary/jap/entry/change-status/' + this.currentWid, {});
      ...
      async removeEdit() {
        const resp = await sendDeleteRequest('dictionary/jap/entries/' + this.currentWid);
          if (resp.status == 200 && resp.data.id !== 0) {
      ...
      async saveEdit() {
        ...
        if (this.currentWid) {
          const resp = await sendPostRequest('dictionary/jap/entries/' + this.currentWid, newEntry);
          if (resp.status == 200 && resp.data.id !== 0) {
    ...
    components: {
        JapEntryView, JapEntryEdit, JapEntryTextEditor, DictionaryIcon, DuplicatesChecker
    }
}
</script>
