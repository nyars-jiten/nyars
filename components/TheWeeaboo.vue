...
<template>
  <section class="flex flex-wrap">
    <div v-for="{ username, avatar, userRating } of users" class="flex p-4 md:w-1/3">
      <img
      ...
      <div class="grow pl-6">
        ...
        <div class="grid grid-cols-[1fr_min-content] gap-x-4">
          <div>{{ $t('stats.week_rating') }}</div>
          <div>{{ userRating.weekRating }}</div>

          <div>{{ $t('stats.summary_new') }}</div>
          <div>{{ userRating.summary.week.japNew }}</div>

          <div>{{ $t('stats.summary_edit') }}</div>
