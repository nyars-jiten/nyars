...
<template>
  <section class="relative rounded-md bg-white shadow-md dark:bg-gray-800">
    ...
    >
      ...
      <div class="group relative z-20 grow">
        <input
          ...
          :placeholder="$t('search.input.jpn')"
          @keydown.enter="searchImmediately"
          @keydown.escape="close"
        />

        <TheSuggs @on-selected="searchImmediately" />
      </div>
    ...
    >
      ...
      >
        <span class="text-gray-500">{{ $t('search.in') }}</span>

        <RadioGroup v-model="mode" class="flex flex-row gap-2">
          <RadioGroupOption v-slot="{ checked }" :value="SearchType.Jpn">
            <Button :disabled="checked">
              {{ $t(`search.what.${SearchType.Jpn}.short`) }}
            </Button>
          </RadioGroupOption>
          <RadioGroupOption v-slot="{ checked }" :value="SearchType.Kanji">
            <Button :disabled="checked">
              {{ $t(`search.what.${SearchType.Kanji}.short`) }}
            </Button>
          </RadioGroupOption>
        </RadioGroup>
      </div>
