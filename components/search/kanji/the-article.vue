  ...
  >
    <div class="grid select-text grid-cols-1 gap-8 md:grid-cols-[12rem_1fr]">
      <div class="flex flex-col gap-8">
        ...
        >
          <div class="flex flex-col gap-2">
            ...
            <ContentCopyIcon
              v-if="standalone"
              :size="16"
              class="invisible absolute bottom-2 right-2 opacity-50 group-hover:visible"
            />
          </div>
        </Component>
        ...
        <div
          v-if="standalone"
          class="flex flex-col gap-2 rounded-md bg-neutral-100 px-3 py-2 dark:bg-gray-700"
        >
      ...
      <div class="flex flex-col gap-8">
        ...
        <div v-if="article.entry.readings" class="flex flex-col gap-4">
          <template v-for="(reading, readingType) of article.entry.readings">
            <TheReading
              v-if="
                (standalone || !['other', 'nanori'].includes(readingType)) && reading.length > 0
              "
              :readings="reading"
              :type="readingType"
              :is-editor="false"
        ...
        <div v-if="standalone" class="flex flex-col gap-8">
          ...
          <TheIndices v-if="article.entry.indices.length > 0" :indices="article.entry.indices" />
        </div>
      </div>
    </div>
