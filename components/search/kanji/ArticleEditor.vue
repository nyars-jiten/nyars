  ...
  >
    <div class="grid select-text grid-cols-1 gap-8 md:grid-cols-[12rem_1fr]">
      <div class="flex flex-col gap-8">
        ...
        <div class="flex flex-col gap-2 rounded-md bg-neutral-100 px-3 py-2 dark:bg-gray-700">
          ...
          <p>
            ...
            <EditInput v-model="article.entry.general.literal" name="literal-input" />
          </p>
          <p>
            <span class="text-sm text-gray-400">
              {{ t(MessagesNames.StrokeCount) }}
            </span>
            <EditInput v-model="article.entry.general.strokeCount" name="strokecount-input" />
          </p>
          <p>
            <span class="text-sm text-gray-400">
              {{ t(MessagesNames.Ids) }}
      ...
      <div class="flex flex-col gap-8">
        ...
        <div v-if="article.entry.readings" class="flex flex-col gap-4">
          <template v-for="(reading, readingType) of article.entry.readings">
            <TheReading :readings="reading" :type="readingType" :is-editor="true" />
          </template>
        </div>

        <div class="flex flex-col gap-2 rounded-md bg-neutral-100 px-3 py-2 dark:bg-gray-700">
          <span class="text-sm text-gray-400">{{ t(MessagesNames.Note) }}</span>
          <TextArea v-model="article.entry.general.note" :rows="4" name="note-input" />
        </div>
